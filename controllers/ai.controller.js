...
// Phiên bản này chứa cả 2 bộ não AI cho Kế toán và Khuyến mãi.
// ==========================================================
...
async function analyzeDailyReport(req, res) {
  ...
  try {
    ...
    const todaysOrders = await Order.find({
      created_at_haravan: { $gte: reportDate, $lt: nextDay }
    });
    ...
    const prompt = `
      Là một chuyên gia phân tích kinh doanh, hãy phân tích kết quả kinh doanh hôm nay và đề xuất kế hoạch cho ngày mai.
      Dữ liệu:
      ...
      - Top sản phẩm bán chạy: ${JSON.stringify(Object.entries(topProducts).sort((a, b) => b[1] - a[1]).slice(0, 5))}
      - Mã giảm giá đã dùng: ${JSON.stringify(usedCoupons)}

      Trả về một đối tượng JSON có cấu trúc:
      {
        "summary": "Tóm tắt (2-3 câu) về tình hình kinh doanh hôm nay.",
        "insights": ["Nhận định 1 về doanh thu/lợi nhuận.", "Nhận định 2 về sản phẩm/mã giảm giá."],
        "recommendations": [
          { "action": "Đề xuất hành động 1.", "details": "Chi tiết hành động 1.", "reason": "Lý do." },
          { "action": "Đề xuất hành động 2.", "details": "Chi tiết hành động 2.", "reason": "Lý do." }
        ]
      }
    `;
